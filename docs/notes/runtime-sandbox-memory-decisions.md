...
- Sandbox 由独立 Worker 通过 Docker/runc 启动。命令容器默认使用普通 Docker bridge 出网，同时保留 `networkMode=deny`；只读根文件系统、删除 capabilities、启用 `no-new-privileges`，并限制 PID、CPU、内存和时长；Session 工作区单独持久化。它适合当前 beta 和受控租户，不应被宣传成敌对多租户的最终隔离边界。
...
Environment 也不再接受 secret 变量。2026-07-15 复核火山方舟后需要把“全局”拆成两层：Vault 是账号/租户级可发现资源池，但 Credential 的使用仍由 MCP/Skill binding 选择，代理层动态解密；它不是每个 Session 都得到一份明文。雪山方舟采用同一语义。模型 Provider 的 Endpoint 与密钥改由平台管理员单独管理，普通 Agent 作者只选择模型目录项；历史 `Agent.model.credentialId` 仅为兼容 BYO 接入保留，不再出现在普通用户界面。

## “禁用网络”此前究竟禁了什么

旧实现中的 `--network none` 只作用于 Worker 为 `bash/glob/grep` 拉起的短命 Docker 命令容器：

- `read/write/edit` 由 Worker 直接操作 Session Workspace；
- `web_fetch` 由 Worker 进程发起 HTTP；
- MCP 的 `tools/list` 与调用由 API 内的 MCP Proxy 发起；
- 模型请求由 Harness/API 发起。

所以“Sandbox 禁网”从来不等于 Managed Agent 整体断网。2026-07-15 起命令容器默认也允许普通 bridge 出网，`deny` 只作为高风险 Session 的可选硬边界。后续若要提供域名 allowlist，必须实现真正的出口代理/策略网关，不能把一个未执行的字符串列表包装成安全能力。
