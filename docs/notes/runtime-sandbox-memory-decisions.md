...
Environment 也不再接受 secret 变量。Vault 本身不是默认对所有 Agent/Session 开放的全局命名空间：模型 Credential 由 `Agent.model.credentialId` 引用，MCP Credential 由 `Agent.mcpServers[].credentialId` 引用；Session 固定 Agent Version 后间接固定这些非秘密引用。只有控制面代理可以解密，Environment、Session 记录和 Sandbox 都不能读取明文。管理员配置的 `MODEL_API_KEY` 是独立的系统默认模型配置，不属于 Vault。
