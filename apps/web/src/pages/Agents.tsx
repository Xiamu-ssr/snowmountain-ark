...
export function AgentCreatePage() {
  ...
  const submit = async () => {
    ...
    try {
      const model = provider === "mock" ? { provider, name: modelName } : { provider, name: modelName, baseUrl, ...(modelCredentialId ? { credentialId: modelCredentialId } : {}) };
  ...
  return <div className="page page-narrow">
    ...
    <section className="form-panel">
      ...
      <div className="form-grid three"><label>Provider<select value={provider} onChange={(event) => setProvider(event.target.value as typeof provider)}><option value="mock">Local deterministic（无 LLM）</option><option value="openai-compatible">OpenAI-compatible</option></select></label><label>模型 / Harness ID<input value={modelName} onChange={(event) => setModelName(event.target.value)} /></label><label>Base URL<input disabled={provider === "mock"} value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder="https://.../v1" /></label></div>
      {provider === "openai-compatible" && <label>模型 Credential <span>显式绑定；不选择时只会使用管理员配置的系统默认模型 Key</span><select value={modelCredentialId} onChange={(event) => setModelCredentialId(event.target.value)}><option value="">系统默认（如已配置）</option>{credentials.filter((credential) => !credential.usage || ["model", "generic"].includes(credential.usage)).map((credential) => <option value={credential.id} key={credential.id}>{credential.name} · {credential.vaultId}</option>)}</select></label>}
...
export function AgentEditPage() {
  ...
  useEffect(() => {
    Promise.all([api.get<Agent>("agents", id), api.list<Agent>("agents"), api.list<Credential>("credentials"), api.market()])
      .then(([current, allAgents, allCredentials, catalog]) => {
        ...
        setProvider(current.model.provider); setModelName(current.model.name); setBaseUrl(current.model.baseUrl ?? ""); setModelCredentialId(current.model.credentialId ?? ""); setToolPolicies(current.toolPolicies);
  ...
  const save = async () => {
    ...
    try {
      const next = await api.patch<Agent>("agents", id, {
        ...
        model: { provider, name: modelName, ...(provider === "openai-compatible" && baseUrl ? { baseUrl } : {}), ...(provider === "openai-compatible" && modelCredentialId ? { credentialId: modelCredentialId } : {}) },
  ...
  return <div className="page page-narrow">
    ...
    <section className="form-panel">
      ...
      <StepTitle number="02" title="模型配置" /><div className="form-grid three"><label>Provider<select value={provider} onChange={(event) => setProvider(event.target.value as typeof provider)}><option value="mock">Local deterministic（无 LLM）</option><option value="openai-compatible">OpenAI-compatible</option></select></label><label>模型 / Harness ID<input value={modelName} onChange={(event) => setModelName(event.target.value)} /></label><label>Base URL<input disabled={provider === "mock"} value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label></div>{provider === "openai-compatible" && <label>模型 Credential <span>由 Agent Version 显式引用</span><select value={modelCredentialId} onChange={(event) => setModelCredentialId(event.target.value)}><option value="">系统默认（如已配置）</option>{credentials.filter((credential) => !credential.usage || ["model", "generic"].includes(credential.usage)).map((credential) => <option value={credential.id} key={credential.id}>{credential.name} · {credential.vaultId}</option>)}</select></label>}
