...
function EnvironmentModal({ open, form, setForm, onClose, onSave, title }: {
  ...
  return <Modal title={title} open={open} onClose={onClose} footer={<><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" disabled={!form.name} onClick={onSave}>保存</button></>}>
    ...
    <div className="security-note"><LockKeyhole size={18} /><div><strong>权限和秘密都不属于 Environment</strong><p>文件系统、网络和资源上限由 Sandbox/Policy 实施；模型和 MCP 凭证必须在 Agent 上显式绑定 Vault Credential。</p></div></div>
  </Modal>;
}
...
export function VaultsPage() {
  ...
  const filtered = items.filter((item) => `${item.name} ${item.id}`.toLowerCase().includes(search.toLowerCase()));
  return <div className="page"><PageHeader title="Credentials Vault" description="Vault 只负责加密保存；模型与 MCP 必须在 Agent Version 中显式引用 Credential ID。" action={<button className="button primary" onClick={() => setOpen(true)}><Plus size={16} />创建 Vault</button>} />{error && <ErrorBanner error={error} />}<div className="vault-banner"><Shield size={22} /><div><strong>AES-256-GCM at rest · proxy-only resolution</strong><p>Vault 不绑定 Environment 或 Session；明文不进入模型上下文或 Sandbox。生产环境应接入云 KMS/HSM 轮换。</p></div><CheckCircle2 size={18} /></div><section className="panel"><Toolbar search={search} onSearch={setSearch} />{filtered.length ? <div className="card-grid">{filtered.map((item) => <Link className="vault-card" to={`/vaults/${item.id}`} key={item.id}><div className="vault-card-head"><span className="resource-icon amber"><KeyRound size={17} /></span><ChevronRight size={16} /></div><h3>{item.name}</h3><p>{item.description || "集中托管凭证"}</p><small>{item.id}</small><div className="vault-card-foot"><span><ServerCog size={14} />{credentials.filter((value) => value.vaultId === item.id).length} Credentials</span><span>{new Date(item.updatedAt).toLocaleDateString()}</span></div></Link>)}</div> : <EmptyState title="暂无 Vault" description="创建 Vault 后再添加模型或 MCP Credential。" />}</section><Modal title="创建 Vault" open={open} onClose={() => setOpen(false)} footer={<><button className="button secondary" onClick={() => setOpen(false)}>取消</button><button className="button primary" onClick={() => void create()} disabled={!name}>创建 Vault</button></>}><label>名称<input value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：production-credentials" /></label><div className="security-note"><LockKeyhole size={18} /><div><strong>Vault 不是隐式全局权限</strong><p>创建 Credential 后仍需在 Agent Version 中显式选择；修改时必须重新填写并校验。</p></div></div></Modal></div>;
}
...
export function VaultDetailPage() {
  ...
  const [error, setError] = useState("");
  const [form, setForm] = useState({ name: "", description: "", serverUrl: "", mcpServerName: "", usage: "mcp" as "mcp" | "model" | "generic", authType: "bearer" as Credential["authType"], secret: "", clientId: "", clientSecret: "", tokenUrl: "", scopeText: "" });
  ...
  const validate = async () => { setValidating(true); setError(""); try { const result = await api.validateCredential(credentialPayload()); setValidated(result.valid); if (!result.valid) setError(`${form.usage === "model" ? "模型 Endpoint" : "MCP Server"} 未通过校验`); } catch (reason) { setValidated(false); setError(reason instanceof Error ? reason.message : String(reason)); } finally { setValidating(false); } };
  ...
  return <div className="page"><div className="detail-hero"><Link className="back-link" to="/vaults">Credentials Vault</Link><ChevronRight size={15} /><span>{vault.name}</span><div className="detail-actions"><button className="button primary" onClick={() => setOpen(true)}><Plus size={15} />添加 Credential</button></div></div>{error && <ErrorBanner error={error} />}<section className="identity-card"><span className="resource-icon large amber"><KeyRound size={24} /></span><div><h1>{vault.name}</h1><p>{vault.description || "集中托管凭证"}</p><small>{vault.id} · 更新于 {new Date(vault.updatedAt).toLocaleString()}</small></div></section><section className="panel resource-detail-table">{credentials.length ? <div className="table-wrap"><table><thead><tr><th>名称 / ID</th><th>用途</th><th>Endpoint URL</th><th>鉴权方式</th><th>校验状态</th><th>更新时间</th><th /></tr></thead><tbody>{credentials.map((credential) => <tr key={credential.id}><td><span className="resource-link"><span className="resource-icon amber"><KeyRound size={15} /></span><span><strong>{credential.name}</strong><small>{credential.id}</small></span></span></td><td>{credential.usage ?? "mcp"}</td><td><code>{credential.serverUrl}</code></td><td>{credential.authType}</td><td><span className={`spec-status ${credential.validationStatus === "valid" ? "implemented" : "reference"}`}>{credential.validationStatus ?? "unvalidated"}</span></td><td>{new Date(credential.updatedAt).toLocaleString()}</td><td><button className="icon-button danger" onClick={() => void remove(credential.id)}><Trash2 size={15} /></button></td></tr>)}</tbody></table></div> : <EmptyState title="暂无 Credential" description="创建模型或 MCP Credential，并在 Agent Version 中显式引用。" />}</section>
    <Modal title="创建 Credential" open={open} onClose={() => setOpen(false)} footer={<><button className="button secondary" onClick={() => setOpen(false)}>取消</button><button className="button secondary" disabled={!form.serverUrl || validating} onClick={() => void validate()}>{validating ? "校验中…" : validated ? "已校验" : "校验"}</button><button className="button primary" disabled={!form.name || !form.serverUrl || !validated} onClick={() => void create()}>创建</button></>}>
      <div className="security-note"><LockKeyhole size={18} /><div><strong>保存后明文不可再次查看</strong><p>Vault 不会自动应用到所有 Session；创建后必须在 Agent 的模型或 MCP binding 中显式选择 Credential ID。</p></div></div>
      <div className="mode-toggle credential-auth"><button className={form.usage === "mcp" ? "active" : ""} onClick={() => { setForm({ ...form, usage: "mcp" }); setValidated(false); }}>MCP</button><button className={form.usage === "model" ? "active" : ""} onClick={() => { setForm({ ...form, usage: "model" }); setValidated(false); }}>模型</button><button className={form.usage === "generic" ? "active" : ""} onClick={() => { setForm({ ...form, usage: "generic" }); setValidated(false); }}>通用</button></div>
      ...
      {form.usage !== "model" && <label>MCP Server 名称<input value={form.mcpServerName} onChange={(event) => { setForm({ ...form, mcpServerName: event.target.value }); setValidated(false); }} /></label>}
      <label>{form.usage === "model" ? "模型 Base URL" : "MCP Server URL"}<input value={form.serverUrl} onChange={(event) => { setForm({ ...form, serverUrl: event.target.value }); setValidated(false); }} placeholder={form.usage === "model" ? "https://api.example.com/v1" : "https://mcp.example.com/mcp"} /></label>
